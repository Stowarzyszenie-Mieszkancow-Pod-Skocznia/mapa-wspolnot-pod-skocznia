#!/usr/bin/env python3
"""
Pobiera pozwolenia na budowę z bazy Oracle MapViewer (dane_wawa) i zapisuje
wynik jako layers/overlays/data/pozwoleniaBudGeoJSON.js.

Użycie:
  python fetch_pozwolenia_bud.py [--bbox "minLng,minLat,maxLng,maxLat"] [--dry-run]
"""
...
DEFAULT_OUT  = Path(__file__).parent.parent.parent / "layers/overlays/data/pozwoleniaBudGeoJSON.js"

logging.basicConfig(level=logging.INFO, format="%(levelname)s  %(message)s")
log = logging.getLogger("fetch_pozwolenia_bud")
...
def fetch_pozwolenia(session: requests.Session, bbox: str) -> list[dict]:
  ...
  ID_PB, DEC_PB, DEC_WZ,
  DATA_PB,
  TYP_INW, NAZWA_INW, OPIS_RODZ_,
  NAZWA_UL, NR_P, NAZWA_WN,
  POW_TER, POW_ZAB, POW_CA, POW_UZ, POW_USL,
  KOND, WYSOKOSC, LICZBA_MIE, LICZBA_PAR,
  UWAGI,
  TO_CHAR(DBMS_LOB.SUBSTR(GEOMETRY_JSON_WGS, 3500, 1)) AS GEOM_JSON
FROM POZWOLENIA_BUD_POW
WHERE ID_PB IS NOT NULL
AND SDO_FILTER(SHAPE, {bbox_geom}, 'querytype=WINDOW') = 'TRUE'"""

    log.info("Pobieranie pozwoleń na budowę z bbox %s…", bbox)
    rowset = info_request(session, sql)
    rows = rowset.findall("ROW")
    log.info("  Pobrano %d pozwoleń.", len(rows))
    ...
    for row in rows:
        ...
        try:
            ...
            log.warning("  Pominięto ID_PB=%s: błąd parsowania geometrii", _text(row, "ID_PB"))
            skipped += 1
            continue

        props = {
            "fid":       _text(row, "ID_PB"),
            "dec_pb":    _text(row, "DEC_PB"),
            "dec_wz":    _text(row, "DEC_WZ"),
            "data":      _text(row, "DATA_PB"),
            "typ_inw":   _text(row, "TYP_INW"),
            "nazwa":     _text(row, "NAZWA_INW"),
            "rodzaj":    _text(row, "OPIS_RODZ_"),
            "ulica":     _text(row, "NAZWA_UL"),
            "nr":        _text(row, "NR_P"),
            "inwestor":  _text(row, "NAZWA_WN"),
            "pow_ter":   _num(row, "POW_TER"),
            "pow_zab":   _num(row, "POW_ZAB"),
            "pow_ca":    _num(row, "POW_CA"),
            "pow_uz":    _num(row, "POW_UZ"),
            "pow_usl":   _num(row, "POW_USL"),
            "kondygnacje": _num(row, "KOND"),
            "wysokosc":  _num(row, "WYSOKOSC"),
            "mieszkania": _num(row, "LICZBA_MIE"),
            "parkingi":  _num(row, "LICZBA_PAR"),
            "uwagi":     _text(row, "UWAGI"),
        }
    ...
    if skipped:
        log.warning("  Pominięto %d pozwoleń (brak geometrii).", skipped)
    return features


def write_js(path: Path, features: list[dict], dry_run: bool) -> None:
    lines = [
        "const pozwoleniaBudGeoJSON = {",
    ...
    lines += ["  ]", "}", "", "export { pozwoleniaBudGeoJSON }"]
    content = "\n".join(lines)
    size_kb = len(content.encode("utf-8")) // 1024
    if dry_run:
        log.info("[dry-run] %s: %d pozwoleń, %d KB", path.name, len(features), size_kb)
    else:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(content, encoding="utf-8")
        log.info("Zapisano %s (%d pozwoleń, %d KB)", path, len(features), size_kb)


def main():
    ap = argparse.ArgumentParser(description="Pobiera pozwolenia na budowę z Oracle MapViewer")
    ...
    session.headers["User-Agent"] = "fetch-pozwolenia-bud/1.0 (mapaPodSkocznia)"

    features = fetch_pozwolenia(session, args.bbox)
